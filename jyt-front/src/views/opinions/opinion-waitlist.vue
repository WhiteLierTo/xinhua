  ...
  <div class="containerbox">
    ...
    <div class="draftlist">
      <el-row class="cardlist" v-for="(item, index) in waitList" :key="index">
        <el-card>
          <el-row type="flex" align="middle" @click.native="openOrder(item.orderId, item.processId)">
            <el-col :span="22">
              <el-row class="itemtitle" type="flex" align="middle">
                <el-tag type="success" effect="plain" size="mini" style="margin-right: 10px;" v-if="item.readFlag == 0">未读</el-tag>
                {{ item.artTitle }}
              </el-row>
              ...
              <el-row class="itemkey">
                <el-col :span="4" v-if="item.lastOperatorDeptName != null && item.lastOperatorDeptName != ''" class="dept">{{item.lastOperatorDeptName}}</el-col>
                ...
                <el-col :span="2" v-if="item.timeStatus != null && item.timeStatus == 1" class="chaoqi">即将超期</el-col>
                <el-col :span="2" v-if="item.timeStatus != null && item.timeStatus == 2" class="chaoqi">已超期</el-col>
...
export default {
  ...
  methods: {
    ...
    openOrder(id, processId) {
      this.$router.push('/waitDetail/' + id + '/' + this.queryInfo.searchType + '/' + this.highlight + '/' + processId);
    },
    search() {

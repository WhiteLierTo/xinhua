<template>
  <div class="containerbox">
          ...
          <el-row type="flex" align="middle" @click.native="openOrder(item.orderId, item.processId)">
            <el-col :span="22">
              ...
              <el-row class="itemkey">
                ...
                <el-col :span="3" class="yuqleibie">{{ item.opinionTypeName }}</el-col>
              </el-row>
            </el-col>
    ...
    <el-row type="flex" align="middle" style="height: 58px;" v-if="total > 0">
      <el-col :span="24">
        <BizPagination :total="total" :page.sync="queryInfo.pageNum" :limit.sync="queryInfo.pageSize" @pagination="getLeaderList" />
      </el-col>
    </el-row>
  </div>
</template>
...
<script>
import { getLeaderList } from '@/api/opinion/data';
export default {
  data() {
    return {
      highlight: 5,
  ...
  methods: {
    getLeaderList() {
      getLeaderList(this.queryInfo)
        .then(res => {
          if (res.code == 200) {
            this.total = res.total;
            this.waitList = res.rows;
          } else {
            this.$message.error('获取待批示列表失败');
          }
        })
        .catch(() => {
          this.$message.error('获取待批示列表失败');
    ...
    openOrder(id) {
      this.$router.push('/waitDetail/' + id + '/' + this.queryInfo.searchType + '/' + this.highlight + '/-1')
    },
    search() {

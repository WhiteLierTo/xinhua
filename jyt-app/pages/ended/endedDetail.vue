	...
	<view class="detail-content">
		...
		<view class="process-details" v-if="remindInfo.length > 0">
			...
			<view>
				...
				<uni-collapse class="detail-collapse">
					<uni-collapse-item :title="item.warnDeptName" :content="item.createTime" :operate="item.warnTye == 1 ? '催办' : '督办'" v-for="(item, index) in remindInfo" :key="index">
						<view class="item-cont">
		...
		<!-- 操作-->
		<view class="work" v-if="search != 1"><button @click="exportReport" :loading="isLoading" :disabled="isLoading">导出报告</button></view>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		exportReport() {
			...
			that.isLoading = true
			uni.downloadFile({
				url: config.ROOTPATH + "/common/export/" + this.baseInfo.orderId,
				success: (res) => {
					if (res.statusCode == 200) {
						...
						uni.saveFile({
							...
							fail: (re) => {
								that.$toast.showFail("下载文件异常，请联系管理员")
							}
						})
					} else {
						that.$toast.showFail("下载文件异常，请联系管理员")
